<template>
  <div class="finalizacoes">
    <b-table
      id="tabela-de-finalizacoes"
      :ref="'tabela-de-finalizacoes'"
      :fields="fields"
      :items="filas"
      :responsive="true"
      class="
    ...
    >
      ...
      <template v-slot:cell(CPC)="slot">
        <b-form-checkbox
          :id="slot.item.finalizacao + '_produtiva'"
          v-model="slot.value"
      ...
      <template v-slot:cell(CPCA)="slot">
        <b-form-checkbox
          :id="slot.item.finalizacao + '_obrigatoria'"
          v-model="slot.value"
          :unchecked-value="false"
          :value="true"
          disabled
      ...
      <template v-slot:cell(status)="slot">
        <b-form-checkbox
          :id="slot.item.finalizacao + '_ativa'"
          v-model="slot.value"
          :unchecked-value="false"
          :value="true"
          disabled
      ...
      <template v-slot:cell(add)="slot">
        <b-button
          :id="slot.item.finalizacao + '_edit'"
          v-b-modal="slot.item.finalizacao + '_edit_modal'"
          class="edit-btn"
          variant="outline"
          v-html="editIcon"
        />
        <b-btn
          :id="slot.item.finalizacao + '_add'"
          v-b-modal="slot.item.finalizacao + '_delete'"
          class="add-btn"
          variant="outline"
          v-html="deleteIcon"
        />
    ...
    <div v-for="(i, index) in filas" :key="i.finalizacao + '_edit'">
          ...
          <b-col cols="14">
            ...
            <b-row>
              <b-col class="finalizacao-body-container" cols="3">
                <b-form-input
                  :id="i.finalizacao + '_edit_row_pausa'"
                  :ref="i.finalizacao + '_edit_row_pausa'"
                  v-model="editRowInput.finalizacao"
                  :presentState="i"
                  type="text"
                />
              </b-col>
              ...
              <b-col class="finalizacao-body-container" cols="2">
                <b-form-input
                  :id="i.finalizacao + '_edit_row_pausa'"
                  v-model="editRowInput.descricao"
                  :presentState="i"
                  type="text"
                />
              </b-col>
              <b-col class="produtiva-body-container" cols="1">
                <b-form-checkbox
                  :id="i.finalizacao + '_edit_row_produtiva'"
                  v-model="editRowInput.CPC"
                  :presentState="i"
                  :unchecked-value="false"
                  :value="true"
                />
              ...
              <b-col class="obrigatoria-body-container" cols="1">
                <b-form-checkbox
                  :id="i.finalizacao + '_edit_row_obrigatoria'"
                  v-model="editRowInput.CPCA"
                  :presentState="i"
                  :unchecked-value="false"
                  :value="true"
                />
              ...
              <b-col class="obrigatoria-body-container" cols="1">
                <b-form-checkbox
                  :id="i.finalizacao + '_edit_row_obrigatoria'"
                  v-model="editRowInput.alega_pgto"
                  :presentState="i"
                  :unchecked-value="false"
                  :value="true"
                />
              ...
              <b-col class="obrigatoria-body-container" cols="1">
                <b-form-checkbox
                  :id="i.finalizacao + '_edit_row_obrigatoria'"
                  v-model="editRowInput.promessasv"
                  :presentState="i"
                  :unchecked-value="false"
                  :value="true"
                />
              ...
              <b-col class="ativa-body-container" cols="1">
                <b-form-checkbox
                  :id="i.finalizacao + '_edit_row_ativa'"
      ...
      <b-modal
        ...
        @ok="deleteRow(i.finalizacao, i.data)"
      >
        ...
        <b-col cols="12">
          ...
          <b-row>
            <b-col class="finalizacao-body-container" cols="3">
              <b-form-input
                :id="'new_row_pausa'"
                v-model="newRowInput.finalizacao"
                type="text"
              ></b-form-input>
            </b-col>
            <b-col class="finalizacao-body-container" cols="2">
            ...
            <b-col class="obrigatoria-body-container" cols="1">
              <b-form-checkbox
                :id="'new_row_alega_pgto'"
                v-model="newRowInput.alega_pgto"
                :unchecked-value="false"
                :value="true"
              />
            </b-col>
            <b-col class="obrigatoria-body-container" cols="1">
              <b-form-checkbox
                :id="'new_row_promessasv'"
                v-model="newRowInput.promessasv"
                :unchecked-value="false"
                :value="true"
              />
            </b-col>
...
export default {
  ...
  methods: {
    async deleteFinalization(id){
      let s = await axios.delete(`${baseApiUrl}/finalizations/${id}`);
      console.clear();
    ...
    cancelDelete(p) {
      let toast = {
        ...
        message:
          "Finalização " +
          p.toUpperCase() +
          " foi mantida. A finalização foi cancelada pelo usuário.",
      };
      this.validateAndToast(toast);
    },
    okayAdd() {
      ...
      if (newPausa.length > 0) {
        ...
        let toast = {
          ...
          message:
            "Nova Finalização " +
            newPausa.toUpperCase() +
            " não foi adicionada. Não é possível adicionar Finalizações sem nome ou com o nome em branco. A operação foi cancelada.",
        };
        this.validateAndToast(toast);
      }
    },
      ...
      if (p.length > 0) {
        ...
        let toast = {
          ...
          message:
            "Finalização " +
            p.toUpperCase() +
            " não foi modificada. Não é possível atualizar uma finalização apagando seu nome ou deixando apenas espaços em branco. A operação foi cancelada.",
        };
        this.validateAndToast(toast);
      }
    },
...
.finalizacao-body-container,
.produtiva-body-container,
.obrigatoria-body-container,
.alerta-body-container,
.limite-body-container,
.icone-body-container,
.ativa-body-container {
  display: flex;
  justify-content: center;
  align-content: center;
  flex-wrap: wrap;
...
.tabela-finalizacoes > .table.b-table > thead > tr > [aria-sort="none"],
.tabela-finalizacoes > .table.b-table > tfoot > tr > [aria-sort="none"] {
...
.tabela-finalizacoes > .table.b-table > thead > tr > [aria-sort="ascending"],
.tabela-finalizacoes > .table.b-table > tfoot > tr > [aria-sort="ascending"] {
  background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' width='101' height='101' view-box='0 0 101 101' preserveAspectRatio='none'%3e%3cpath fill='white' d='M51 1l25 23 24 22H1l25-22z'/%3e%3cpath fill='white' opacity='.3' d='M51 101l25-23 24-22H1l25 22z'/%3e%3c/svg%3e");
}

.tabela-finalizacoes > .table.b-table > thead > tr > [aria-sort="descending"],
.tabela-finalizacoes > .table.b-table > tfoot > tr > [aria-sort="descending"] {
...
.tabela-finalizacoes > .table.b-table > tbody > tr > [aria-colindex="8"],
.tabela-finalizacoes > .table.b-table > thead > tr > [aria-colindex="8"] {
  width: 3.5%;
  text-align: center;
}

.tabela-finalizacoes > .table.b-table > tbody > tr > [aria-colindex="7"],
[aria-colindex="6"],
.tabela-finalizacoes > .table.b-table > thead > tr > [aria-colindex="7"],
[aria-colindex="6"] {
  width: 3%;
  text-align: center;
  justify-content: center;
...
.tabela-finalizacoes > .table.b-table > tbody > tr > [aria-colindex="5"],
[aria-colindex="4"],
.tabela-finalizacoes > .table.b-table > thead > tr > [aria-colindex="5"],
[aria-colindex="4"] {
  width: 14%;
  text-align: center;
}

.tabela-finalizacoes > .table.b-table > tbody > tr > [aria-colindex="3"],
[aria-colindex="2"],
.tabela-finalizacoes > .table.b-table > thead > tr > [aria-colindex="3"],
[aria-colindex="2"] {
  width: 15%;
  text-align: center;
}

.tabela-finalizacoes > .table.b-table > tbody > tr > [aria-colindex="1"],
.tabela-finalizacoes > .table.b-table > thead > tr > [aria-colindex="1"] {
  width: 15%;
}
...
#editar-finalizacoes > .table.b-table > tbody > tr > [aria-colindex="8"],
#editar-finalizacoes > .table.b-table > thead > tr > [aria-colindex="8"] {
